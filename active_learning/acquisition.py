...
class Acquisition(object):
    ...
    # batch mode with EL , 考虑样本间的相似性，尽量选取不相似的样本
    def get_BEL(self, dataset, model_path, acquire_document_num,
                nsamp=100,
                model_name='',
                returned=False,
                thisround=-1,
        ...
        # 在前 4*acquire_document_num 中选 acquire_document_num 个 （未标记样本足够的话）
        sample_domin = min(4*acquire_document_num, len(new_dataset))
        ...
        for cluster in range(0, acquire_document_num):
            ...
            # 应为严格的升序序列，如0,1,2
            # 但是以防万一还是排序一下
            cluster_delt_arr = [ sample_domin[i] for i in sorted(cluster_indexs) ]
            cur_indices.add(new_datapoints[cluster_delt_arr[0]["id"]])
